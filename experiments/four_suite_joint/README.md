...
This directory contains the self-contained code and runbook for the three
frozen LIBERO-40 experiments below. Historical reports, evaluation outputs,
checkpoints, and generated caches are intentionally excluded from the package.
...
The only authoritative instructions for this LIBERO-40 experiment package are
this README and the checked-in `experiments/four_suite_joint/hpc.env.example`
template. The frozen
`libero40-task-relevant-query-conditioning-release` branch remains available
for exact historical reproduction.
